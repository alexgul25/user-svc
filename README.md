...
- Получение списка подписчиков пользователя (метод `GetFollowers` - вызывается через **API Gateway**, для этого пользователю нужен JWT-токен (можно смотреть подписчиков любых пользователей); также вызывается через **Notification Service** (может получить подписчиков любого пользователя) по внутренней сети при создании нового места в **Wishlist Service**)
